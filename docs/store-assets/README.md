...
## プロモーションタイル（任意・ストアの「プロモーション」欄）

| ファイル | サイズ | 用途 |
|---|---|---|
| `cinemagazer-promo-small-ja.jpg` / `-en.jpg` | 440×280 | プロモーション タイル（小） |
| `cinemagazer-promo-marquee-ja.jpg` / `-en.jpg` | 1400×560 | マーキー プロモーション タイル |

いずれも **JPEG（アルファ無し）** で出力済み（ストア要件「JPEG または 24bit PNG（アルファなし）」を満たす）。
夜明けの映画映像（`shot1-scene.png`）を背景にロゴ・タグライン・対応サービスを配置。
ソースは `marquee-ja.html` / `marquee-en.html` / `small-ja.html` / `small-en.html`。
`render.js` はビューポートを各サイズに変えてレンダリング→ sips で JPEG(アルファ無し)化する。
