  ...
  function nativeImgs(all) {
    try {
      ...
      for (var i = 0; i < imgs.length; i++) {
        var img = imgs[i];
        if (img.closest && img.closest('#cg-overlay')) continue; // 自前コピーを除外
        var r = img.getBoundingClientRect();
        if (r.width < vr.width * 0.3) continue;
        if (r.top < vr.top + vr.height * 0.4) continue;
        if (all) { out.push(img); continue; }
        var cs = getComputedStyle(img);
        if (cs.visibility === 'visible' && cs.display !== 'none' && parseFloat(cs.opacity || '1') > 0.1) out.push(img);
      }
      return out;
    } catch (e) { return []; }
  }
  function subtitleImages() { return nativeImgs(false); }
  function allNativeSubtitleImages() { return nativeImgs(true); }
