# Chrome Web Store 提出ガイド (CinemaGazer 0.2.7)
...
2. **新規アイテムを作成**
   - 「新しいアイテム」→ `cinemagazer-0.2.7.zip` をアップロード
...
- [ ] manifest.json のバージョン (`0.2.7`) が正しいか
...
新バージョンを出す場合：
1. `manifest.json` の `version` を上げる (例: `0.2.7` → `0.3.0`)
2. 新しい ZIP を再パッケージ
