...
```
字幕区間と無音区間で再生速度を動的に切替え、Netflix / Prime Video を高速で鑑賞できる拡張です。WISS2011論文の手法を実装。
```

### 詳細な説明（Description）
```
CinemaGazer は、動画の「字幕がある区間（発話）」と「字幕がない区間（無音/効果音のみ）」を自動判別し、それぞれに異なる再生速度を適用することで、内容理解を保ったまま動画を高速で鑑賞できるようにするChrome拡張です。

【特徴】
- Netflix / Amazon Prime Video の公式字幕タイミング情報を利用
- 発話区間は「理解できる速さ」（既定 1.5×）、無音区間は「飛ばせる速さ」（既定 4.0×）
