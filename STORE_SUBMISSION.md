# Chrome Web Store 提出ガイド (CinemaGazer 0.3.4)
...
2. **新規アイテムを作成**
   - 既存アイテム(CinemaGazer)を更新する場合は「パッケージ」タブから、新規なら「新しいアイテム」→ `cinemagazer-0.3.4.zip` をアップロード
   - パッケージのファイル一覧 (18ファイル, 約63KB。`~/Desktop/claude_work/cinemagazer-0.3.4.zip`)
...
- [ ] manifest.json のバージョン (`0.3.4`) が正しいか
...
新バージョンを出す場合：
1. `manifest.json` の `version` を上げる (例: `0.3.4` → `0.3.5`)
2. 新しい ZIP を再パッケージ
