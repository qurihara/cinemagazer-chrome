...
| Disney+ | 正式サポート | OFF | XHR セグメント化WebVTT (xhr-segmented) + MSE timestampOffset 補正 |
...
| `texttrack` | `<video>.textTracks` の cuechange 観察 → `startTextTrackObserver()` | Prime, Hulu（優先） | NO（ブラウザがメディア時刻で公開済み） |
...
#### Disney+ / Hulu 追加対応
Disney+ は正式サポート（`subtitleStrategy: 'xhr-segmented'`、既定 OFF）。Hulu は試作（`subtitleStrategy: 'texttrack-preferred'`、既定 OFF、popup ラベルに「実験的」明示）。
